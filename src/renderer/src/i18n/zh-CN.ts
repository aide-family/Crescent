const zhCN = {
  ...
  confirm: {
    ...
    deleteProvider: '确认删除这个模型供应商？对应模型配置也会被移除。',
  ...
  connections: {
    ...
    connect: '连接',
    connectionList: '连接列表',
    connectionListDescription: '选择本地默认终端，或切换到一个 SSH 连接终端。',
    ...
    passwordEnvVarDescription: '明文密码为空时，登录会从系统环境变量读取密码；只保存变量名。',
  ...
  terminal: {
    ...
    currentTerminalInstruction:
      '当前终端已经处于可工作的目标环境中。不要重新匹配连接、不要再次 SSH 登录；先分析用户需求，建立工作步骤，然后直接基于当前终端继续执行和验证。',
    commandExecuted: '已执行',
    ...
    passwordPromptDescription: '密码会直接键入对应终端，不会写入对话日志。',
    passwordPromptExpired: '终端已经不在密码输入状态，未键入密码。请重新执行触发密码提示的命令。',
    passwordPromptLabel: '密码或验证码',
    passwordPromptSubmit: '键入终端',
    passwordPromptTitle: '终端正在等待密码输入',
    ...
    postLoginAgentInstruction:
      '当前终端已经完成目标连接登录。不要再次登录、不要切换连接；先按下方需求拆解理解用户目的，再基于当前终端继续完成登录后检查、验证、总结和必要处理，直到用户目标完成。若涉及删除或清理文件，先检查范围和影响，只清理确认可以清除的日志文件，并给出执行结果总结。',
    ...
    breakdownRuleNoFabrication:
      '不要编造凭据、目标、路径、命名空间、主机名或默认值；缺失时使用已配置来源或明确说明需要补充。',
    ...
    breakdownRuleUseSubterminal:
      '需要离开当前终端上下文、跨目标对比或写入本地产物时，使用临时子终端，不污染当前终端。',
  ...
  wiki: {
    ...
    cancelEdit: '取消编辑',
    description: '以 Markdown 存储在当前目录 ./wiki 下，并可被 Agent 检索使用。',
    edit: '编辑知识',
