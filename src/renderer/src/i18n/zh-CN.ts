const zhCN = {
  ...
  connections: {
    ...
    moveToBottom: '置底',
    host: '主机',
    clusterHostRegex: '集群主机模式',
    clusterHostRegexDescription:
      '可选。终端主机名匹配该 glob（如 *.gd17.*）或正则（如 ^node-\\d+$）时，视为已登录到目标环境。',
    clusterHostRegexPlaceholder: '例如 *.gd17.* 或 ^node-\\d+$',
    clusterHostRegexInvalid: '集群主机模式不是有效的 glob 或正则。',
    clusterHostRegexTooLong: '集群主机模式最多 {max} 个字符。',
    clusterHostRegexTooComplex: '集群主机模式过于复杂，请简化。',
    saveMissingName: '请填写连接名称。',
    saveMissingHost: '请填写主机。',
    identityFile: '密钥文件',
    importAsNew: '导入为新连接',
    loginActions: '登录动作',
    ...
    rootPasswordDescription: '用于 sudo 等提权密码提示的自动键入；未配置时会弹出输入框。',
    rootPasswordPlaceholder: '可选；留空则在 sudo 时手动输入…',
    port: '端口',
  ...
  terminal: {
    ...
    inputLocked: '输入已锁定',
    agentExecuting: 'AI 执行中',
    interruptCommand: '中断当前命令 (Ctrl+C)',
    commandRunningHidden: '命令正在隐藏的终端中运行',
    subterminalActive: '运行中',
    subterminalExited: '已退出',
    subterminalAgentRunning: '执行中',
    subterminalAgentDone: '完成',
    subterminalAgentError: '失败',
    subterminal: '子终端',
  ...
  input: {
    ...
    resultTruncatedHint: '结果预览被截断，请打开完整记录查看全部内容。',
    ...
    slashCompact: '压缩上下文',
    slashCompactDescription: '把模型上下文里的旧对话压成摘要；界面历史保留',
    compactContext: '压缩上下文',
  ...
  settings: {
    ...
    workbenchLayoutHint: '立即生效，下次打开沿用。',
  ...
  "mcpServers": {
    ...
    providerEnabledHint: '关闭后配置仍保留，但其模型不会出现在对话选择器中。',
    providerId: '供应商 ID',
    providerList: '供应商列表',
    providerModels: '模型列表',
    providerName: '供应商名称',
