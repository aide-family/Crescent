  ...
  private buildToolGuidanceBlock(mode: string, terminalToolsEnabled: boolean): string {
    const terminalGuidance = terminalToolsEnabled
      ? [
          ...
          'If it is unclear whether the task should run in the current terminal, on a saved Crescent SSH connection, or via a one-shot remote command, ask the user one concise clarifying question before acting. Do not invent a host login path when the target environment is ambiguous.',
          ...
          'When the requested operation clearly targets a non-current host and you already have an interactive remote shell there, run commands there directly. Only use ssh with a concrete remote command when intentionally collecting from another host while keeping the current session undisturbed, preferably via execute_subterminal_command. In PTY mode, password/passphrase/OTP prompts are surfaced to the user for input; if terminal context shows pipe fallback, explain that PTY-capable terminal access is required before interactive remote execution.',
