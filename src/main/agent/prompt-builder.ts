  ...
  private buildToolGuidanceBlock(mode: string, terminalToolsEnabled: boolean): string {
    const terminalGuidance = terminalToolsEnabled
      ? [
          ...
          'When the requested operation targets a non-current host, use ssh with a concrete remote command through execute_terminal_command or execute_subterminal_command. In PTY mode, password/passphrase/OTP prompts are surfaced to the user for input; if terminal context shows pipe fallback, explain that PTY-capable terminal access is required before interactive remote execution.',
