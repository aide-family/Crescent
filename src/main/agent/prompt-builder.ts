...
export class AgentPromptBuilder {
  ...
  buildToolLoopPrompt(input: AgentPromptInput): string {
    return [
      ...
      this.buildToolGuidanceBlock(input.mode, input.terminalToolsEnabled !== false),
  ...
  private buildToolGuidanceBlock(mode: string, terminalToolsEnabled: boolean): string {
    const terminalGuidance = terminalToolsEnabled
      ? [
          'When terminal execution is needed, call execute_terminal_command. Default target is the current/focused terminal in this chat session. When the terminal context lists peer terminals or docked sub-terminals for the same session, pass targetTerminalId to run on a specific peer PTY.',
          'If it is unclear whether the task should run in the current terminal, on a peer terminal already open in this session, on a saved Crescent SSH connection, or via a one-shot remote command, ask the user one concise clarifying question before acting. Do not invent a host login path when the target environment is ambiguous.',
          "When the user asks to login first, or Crescent is already logging into a saved connection, wait for that interactive login to finish and then continue in the resulting session. Prefer an interactive login in the current PTY over non-interactive ssh host 'cmd' one-liners when the user wants to land on a remote shell.",
          'When the requested operation clearly targets a non-current host and a peer terminal for that host is listed in the session inventory, call execute_terminal_command with that peer targetTerminalId. Only use ssh with a concrete remote command when intentionally collecting from another host without using a peer PTY, preferably via execute_subterminal_command. In PTY mode, password/passphrase/OTP prompts are surfaced to the user for input; if terminal context shows pipe fallback, explain that PTY-capable terminal access is required before interactive remote execution.',
          'Terminal wait-mode command tools have watchdog timeouts. If a command times out, it is interrupted with Ctrl+C; treat the timeout output as evidence, adjust the approach, and do not wait indefinitely.',
          'If an operation would leave or disturb the current terminal context, operate on another host/cluster, or compare multiple machines/clusters, prefer a peer terminal already opened in the same Crescent chat session (targetTerminalId from inventory) when the user needs a full tab switch; otherwise call execute_subterminal_command for a docked sub-terminal under the current terminal. Use at most three stable terminalName values and reuse the same name for follow-up commands. Never target a terminal id from another chat session.',
          'For continuous observation such as top, atop, or tail -f, call execute_subterminal_command with mode=detach so the watch keeps running in the docked sub-terminal while you verify other evidence in the main terminal. Sample with read_subterminal_output, then stop with interrupt_subterminal when done. Do not use wait mode for unbounded watchers.',
          ...
          'If sudo, SSH, password, passphrase, OTP, or verification-code input is requested, wait for the user to provide it through the terminal prompt flow. Do not treat the prompt itself as a failure, and do not continue with unrelated checks while the prompt remains unresolved.',
          ...
          'Local file requests take precedence over inferred remote context: when the user says 本地/local/this machine, references local paths such as /etc/hosts, ~, $HOME, or /Users, or pastes local shell output such as "➜  ~ cat /etc/hosts", operate in the local/current terminal context. Treat IP addresses inside pasted file contents as data to edit, not as SSH or cluster targets.',
          ...
          'Never call incomplete wrapper, alias, or placeholder commands. If using a wrapper is truly necessary, include the concrete target and subcommand; otherwise use standard shell, ssh with a concrete remote command, kubectl, or local tools directly.',
          'Before acting, infer the business scenario from the user request, loaded skills, SOP context, terminal context, and available tools: goal, affected target, current context, constraints, required evidence, allowed actions, verification standard, and blocker/escalation path.',
          'When a loaded skill provides workflow, safety, or fallback rules, treat it as scenario knowledge and follow it strictly. Do not detour into unrelated systems after the skill identifies the proper target, scope, or blocked condition.',
          'If a task-specific SOP is named in the user request and no matching knowledge-base SOP content is supplied, explicitly say the named SOP was not available locally, then continue only with the relevant loaded skill or direct evidence.',
          'For operations requests, choose evidence sources that directly correspond to the affected target and requested end state. Use indirect systems only when they are relevant according to the current scenario or loaded Skill/SOP, and stop when they cannot answer the next decision.',
          'When permission errors, missing access, unsupported tool capability, or another blocker prevents the next necessary evidence, summarize the blocker and ask for the required access, input, or explicit escalation approval. Do not keep probing unrelated systems.',
          'Avoid interactive or destructive commands unless the user explicitly asked for them.'
        ]
      : [
          'Terminal tools are not available for this run. Do not suggest or attempt terminal command execution.',
          'Use the available non-terminal tools such as MCP tools, document parsers, local file writer, and knowledge-base tools when they match the user request.',
          'If the requested action requires terminal access but terminal tools are unavailable, explain that limitation and ask the user to run the task in a terminal-enabled context.'
        ]
    ...
    return [
      ...
      'Do not let terminal memory, loaded skills, prior cluster work, or a matching IP in file content override an explicit local file target.',
