...
describe('AgentPromptBuilder', () => {
  ...
  it('guides tool runs toward stepwise closed-loop completion', () => {
    ...
    expect(prompt).toContain('targets a non-current host')
    expect(prompt).toContain('use ssh with a concrete remote command')
    expect(prompt).toContain('password/passphrase/OTP prompts are surfaced')
    ...
    expect(prompt).toContain('infer the business scenario')
    expect(prompt).toContain('treat it as scenario knowledge')
    expect(prompt).toContain('directly correspond to the affected target')
    expect(prompt).toContain('unsupported tool capability')
